...
const char *labels[N_CLASSES] = {"back", "go", "one", "ship"};
...
/* ------------------------------------------------------------------ */
void compute_features(uint16_t total_samples,
                      float   *feat_zcr,
                      float   *feat_ste)
{
    ...
    for (uint8_t i = 1; i < N_FRAMES; i++) {
        if (feat_zcr[i] < min_zcr) min_zcr = feat_zcr[i];
        if (feat_zcr[i] > max_zcr) max_zcr = feat_zcr[i];
    }
    float zcr_range = max_zcr - min_zcr;
    for (uint8_t i = 0; i < N_FRAMES; i++)
        feat_zcr[i] = (zcr_range == 0.000f) ? 0.000f
                                           : (feat_zcr[i] - min_zcr) / zcr_range;

    float min_ste = feat_ste[0], max_ste = feat_ste[0];
    for (uint8_t i = 1; i < N_FRAMES; i++) {
        if (feat_ste[i] < min_ste) min_ste = feat_ste[i];
        if (feat_ste[i] > max_ste) max_ste = feat_ste[i];
    }
    float ste_range = max_ste - min_ste;
    for (uint8_t i = 0; i < N_FRAMES; i++)
        feat_ste[i] = (ste_range == 0.000f) ? 0.000f
                                           : (feat_ste[i] - min_ste) / ste_range;

    /* --- debug print --- */
    printf("ZCR: ");
    for (uint8_t i = 0; i < N_FRAMES; i++) printf("%.3f ", feat_zcr[i]);
    printf("\nSTE: ");
    for (uint8_t i = 0; i < N_FRAMES; i++) printf("%.3f ", feat_ste[i]);
    printf("\n");
}
